...
from sklearn.metrics import completeness_score, homogeneity_score, adjusted_rand_score
import numpy as np
import csv
import utils as ut
...
def clustering_inspection(datadir, indir, encdir, list_model, 
    ...
    for mod in list_model:
        if mod == 'convae':
            with open(os.path.join(encdir, mod + '-avg_vect_scaled.csv')) as f:
                rd = csv.reader(f)
                next(rd)
                mrn_list = [] 
                mtx = []
        ...
        print('\n')
        ent_s, pur_s, n_disf, comp, homo, ari = cluster_evaluation(encdir, mtx,
                                                  disease_class[code],
                                                  HCpar,
                                                  mod,
                                                  code, n_iter)
        par_dict[mod] = [ent_s, pur_s, n_disf, comp, homo, ari] # added completeness and homogeneity
        print('*'*100)
        print('\n')
    return par_dict
...
def cluster_evaluation(encdir, data_mtx, gt_clu,
    ...
    clu, ent, pur, n_disf, comp, homo, ari = _outer_clustering_analysis(
                                        data_mtx, gt_clu, 
                                        HCpar['linkage_clu'], 
                                        HCpar['affinity_clu'])
    ...
    return ent, pur, n_disf, comp, homo, ari
...
def _outer_clustering_analysis(data,
    ...
    # Compute completeness (check if skewed distribution of cluster assignment
    # within each class to single cluster) and homogeneity
    completeness = completeness_score(gt_clu, clusters)
    homogeneity = homogeneity_score(gt_clu, clusters)
    ari = adjusted_rand_score(gt_clu, clusters)

    print('\nAverage Entropy: {0:.2f}'.format(entropy))
    print('Average Purity: {0:.2f}'.format(purity))
    print("Completeness score: {0:.2f}".format(completeness))
    print("Homogeneity score: {0:.2f}".format(homogeneity))
    print("Adjusted Rand score: {0:.2f}".format(ari))
    ...
    return clusters, entropy, purity, len(dis_found), completeness, homogeneity, ari
...
if __name__ == '__main__':
    ...
    for it in range(ut.n_iter):
        ...
        for mod in par_dict:
            ...
            comp_dict.setdefault(mod, list()).append(par_dict[mod][3])
            homo_dict.setdefault(mod, list()).append(par_dict[mod][4])
            ari_dict.setdefault(mod, list()).append(par_dict[mod][5])
    ...
    for mod in ent_dict.keys():
        ...
        comp_dict[mod] = sorted(comp_dict[mod])
        homo_dict[mod] = sorted(homo_dict[mod])
        ari_dict[mod] = sorted(ari_dict[mod])
        ...
        print("Mean Completeness: {0:.3f} (SD = {1:.3f}) -- CI = [{2:.3f}, {3:.3f}]".format(np.mean(comp_dict[mod]),
                                                                np.std(comp_dict[mod]),
                                                                comp_dict[mod][int(0.25 * ut.n_iter)],
                                                                comp_dict[mod][int(0.975 * ut.n_iter)]))
        print("Mean Homogeneity: {0:.3f} (SD = {1:.3f}) -- CI = [{2:.3f}, {3:.3f}]".format(np.mean(homo_dict[mod]),
                                                                np.std(homo_dict[mod]),
                                                                homo_dict[mod][int(0.25 * ut.n_iter)],
                                                                homo_dict[mod][int(0.975 * ut.n_iter)]))
        print("Mean Adjusted Rand: {0:.3f} (SD = {1:.3f}) -- CI = [{2:.3f}, {3:.3f}]".format(np.mean(ari_dict[mod]),
                                                                np.std(ari_dict[mod]),
                                                                ari_dict[mod][int(0.25 * ut.n_iter)],
                                                                ari_dict[mod][int(0.975 * ut.n_iter)]))
